/**
 * 150W, CFL=0.30 (relaxed) — measure metal vs gas velocity separately.
 * This is the critical test: is the Ma=2.74 from gas or metal?
 */
...
int main() {
    ...
    printf("================================================================\n");
    printf("  150W CFL=0.30 (RELAXED) — Gas vs Metal Velocity\n");
    printf("  cs = %.1f m/s\n", cs);
    printf("================================================================\n");
    ...
    config.enable_evaporation_mass_loss=false;  // Path B evap cooling ON
    ...
    config.cfl_velocity_target=0.30f;  // RELAXED
    ...
    printf("  %6s %7s %9s %9s %9s %9s %9s %9s %9s\n",
           "Step", "t[μs]", "T_max", "T_iface",
           "v_gas_max", "v_metal", "v_iface",
           "Ma_gas", "Ma_metal");
    ...
    for (int step=0; step<=5000; step++) {
        ...
        if (step % 500 == 0) {
            ...
            float v_gas_max=0, v_metal=0, v_iface=0;
            float T_iface=0;
            ...
            printf("  %6d %7.1f %9.0f %9.0f %9.1f %9.1f %9.1f %9.3f %9.3f\n",
                   step, step*dt*1e6, solver.getMaxTemperature(), T_iface,
                   v_gas_max, v_metal, v_iface,
                   v_gas_max/cs, v_metal/cs);
        }
    }
    return 0;
}
