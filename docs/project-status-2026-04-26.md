...
| **P5** | Phase-5 集成验证 (emissivity+ray+A合并) | `apps/sim_linescan_phase5.cu` | configured, awaiting A merge |

**Phase-5 配置要点** (2026-04-26, 纯配置, 无 GPU 运行):
- emissivity 0.28 → 0.55 (F3D hflem1=0.55, 任务 D 部分完成)
- ray tracing rays 2048→4096, bounces 3→5, dda_steps 800→1500 (恢复完整预算)
- ν_LU 0.065 不变 (Phase-3 证明降低会恶化中心 Δh)
- VOF 质量修正默认 OFF; 编译 -DPHASE5_VOF_MASS_CORR=1 可启用 (等 A 合并后)
- 域 1100×75×100 = 8.25 M cells, 440 B/cell = 3630 MB (< 4 GB 安全)
- CMakeLists **未注册** — 由人工在 A 合并后添加
- 构建脚本: `scripts/flow3d/run_phase5.sh`
- 基线对比: Phase-1 中心 Δh -16 μm, Phase-4 -22 μm, 目标 > -16 μm
