...
def main():
    ...
    # LE zoom window: ±0.3c around LE (x in [9.7, 10.3])
    nx, ny, nz = found_cases[0][2]
    dx = found_cases[0][3]
    le_x = 10.0; le_y = ny * dx / 2.0
    x_lo_c, x_hi_c = -0.2, +1.2
    y_lo_c, y_hi_c = -0.20, +0.20
    ...
    for row, (label, color, dims, dx_, omega, umag, mask) in enumerate(found_cases):
        ...
        if mask is not None:
            ...
            xs = np.linspace(extent[0], extent[1], mc.shape[1])
            ys = np.linspace(extent[2], extent[3], mc.shape[0])
            for ax in axes[row, :]:
    ...
    fig.suptitle("NACA0012 settled flow field — AMR-OFF vs AMR-ON (bilinear+time interp)\n"
                 "Look for sharper LE suction (darker blue/red at LE) and tighter vortex cores",
                 fontsize=12, y=1.005)
    plt.tight_layout()
