/**
 * @file test_energy_simple.cu
 * @brief Simple standalone test for energy balance computation
 */
...
int main() {
    ...
    // Copy to device
    cudaMemcpy(d_T, h_T, num_cells * sizeof(float), cudaMemcpyHostToDevice);
    cudaMemcpy(d_fl, h_fl, num_cells * sizeof(float), cudaMemcpyHostToDevice);
    ...
    for (int step = 0; step < n_steps; ++step) {
        ...
        double power_error = std::abs(dE_dt_measured - P_input) / P_input * 100.0;
        ...
        std::cout << "Step " << std::setw(2) << step + 1
                  ...
                  << " (error=" << std::setprecision(2) << power_error << "%)" << std::endl;
