...
TEST(MultiphysicsForceDiagnostic, TemperatureGradientGeneratesForce) {
    ...
    config.kinematic_viscosity = 0.1667f;  // Higher viscosity for stability (tau=1.0)
    config.density = 4110.0f;
    config.dsigma_dT = -0.26e-3f;
    config.dt = 1e-7f;

    // Disable CFL limiting — this test validates force pipeline, not CFL behavior
    ...
    // Keep lattice force small: F_lattice = F_phys * dt²/dx
    // With dsigma_dT=-0.26e-3, ΔT=5K over 10 cells (dx=2e-6):
    //   ∇T ~ 5/(10*2e-6) = 2.5e5 K/m
    //   F_phys ~ |dsigma_dT| * ∇T / dx ~ 0.26e-3 * 2.5e5 / 2e-6 ~ 3.25e7
    //   F_lattice ~ 3.25e7 * (1e-7)²/(2e-6) ~ 0.016 (safe)
    int num_cells = config.nx * config.ny * config.nz;
    std::vector<float> h_temp(num_cells);

    float T_hot = 2005.0f;   // K (small gradient → stable lattice force)
    ...
    // With CFL limiting disabled, Marangoni force should produce measurable velocity
    // Threshold lowered from 1e-3: force conversion fix (dt²/(dx*rho)) makes forces
    ...
    EXPECT_GT(final_v, 1e-4f) << "Velocity should be significant (> 0.0001 m/s)";
...
TEST(MultiphysicsForceDiagnostic, UniformTemperatureGeneratesZeroForce) {
    ...
    // Test: Velocity should be near zero (no gradient = no force)
    // Allow small numerical noise from finite precision
    EXPECT_NEAR(final_v, 0.0f, 1e-5f)
