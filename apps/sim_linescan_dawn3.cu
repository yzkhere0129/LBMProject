 ...
 *
 * Usage:
 *   mkdir -p output_dawn3 && ./sim_line_scan_316L
 *   ParaView: Open output_dawn3/line_scan_*.vtk
 */
...
int main() {
    ...
    config.material.dsigma_dT = -4.9e-4f;  // Dawn-3: stress test old "Iron" steeper Marangoni
    ...
    for (int step = 0; step <= num_steps; ++step) {
        ...
        if (step % vtk_every == 0) {
            ...
            snprintf(filename, sizeof(filename),
                     "output_dawn3/line_scan_%06d", step);
    ...
    printf("\nOutput: output_dawn3/line_scan_*.vtk\n");
    printf("============================================================\n\n");
