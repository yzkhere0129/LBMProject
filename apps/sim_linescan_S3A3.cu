...
int main() {
    ...
    // S3-A3: Reduce recoil coefficient from 0.54 (Knight 1979) to 0.3.
    // Hypothesis: recoil pushes melt radially out of keyhole during scan.
    // If the radial recoil overshoot is building side ridges (by pushing
    // melt laterally rather than downward), reducing it should reduce
    // ridge height and allow more mass to stay near center.
    config.marangoni_csf_multiplier = 1.0f;   // unchanged
    config.recoil_coefficient       = 0.3f;   // S3-A3: 0.3 (was 0.54, Knight 1979)
    ...
    for (int step = 0; step <= num_steps; ++step) {
        ...
        if (step % vtk_every == 0) {
            ...
            snprintf(filename, sizeof(filename),
                     "output_S3A3/line_scan_%06d", step);
