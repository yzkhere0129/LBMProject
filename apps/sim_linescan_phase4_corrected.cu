 ...
 *
 * Usage:
 *   mkdir -p output_phase4_corrected && ./sim_line_scan_316L
 *   ParaView: Open output_phase4_corrected/line_scan_*.vtk
 */
...
int main() {
    ...
    config.nx = 1100;
    config.ny = 110;  // Phase-4-corrected: 220μm transverse — wider than Phase-4 (150μm) but fits 4GB with nz=75
    config.nz = 75;   // Phase-4-corrected: 150μm depth (pool 100 + 30 gas above + 20 below clearance)
    config.dx = 2.0e-6f;
    config.dt = 8.0e-8f;
    ...
    config.mass_correction_z_substrate_lu      = 60.0f;    // Phase-4-corrected: matches interface_z=60
    ...
    const float interface_z = 60.0f;  // Phase-4-corrected: substrate top at z=60·dx=120μm
    ...
    // Initialize
    // ==================================================================
    mkdir("output_phase4_corrected", 0755);
    ...
    for (int step = 0; step <= num_steps; ++step) {
        ...
        if (step % vtk_every == 0) {
            ...
            snprintf(filename, sizeof(filename),
                     "output_phase4_corrected/line_scan_%06d", step);
    ...
    printf("\nOutput: output_phase4_corrected/line_scan_*.vtk\n");
    printf("============================================================\n\n");
