 ...
 *
 * Usage:
 *   mkdir -p output_round8_s2 && ./sim_line_scan_316L
 *   ParaView: Open output_round8_s2/line_scan_*.vtk
 */
...
int main() {
    ...
    // Phase-4 final 2 ms run, 4 GB GPU constraint.
    // Laser end position: 500 + 0.8·2000 = 2100 μm; beam tail +3·w₀=117 μm.
    // Per-cell GPU footprint observed in Phase-1: ~440 bytes/cell.
    // Budget: keep below 8.5M cells so total <3700 MB (Phase-1 used 3566 MB).
    // Choice: 1100×80×100 = 8.8M cells (slightly over) → fall to 1100×75×100=8.25M.
    // ny=75 sacrifices wider transverse (Phase-1 had 125 = 250 μm, this = 150 μm)
    ...
    // R8 Stage 1: physics-based bulk boiling cooling for f>=0.99 cells.
    // Replaces the dead-zone between T_boil and T_boil+1500K where bulk cells
    // had no cooling. cfd-math-expert recommends α_boil=2e-4 (R8 Stage 1.2).
    // Tighten subsurface cap from 1500K to 200K (safety floor only, with bulk
    // cooling now primary mechanism).
    config.physics.enable_bulk_boiling_cooling = true;
    config.physics.bulk_boiling_alpha          = 2.0e-4f;
    config.physics.subsurface_cap_overshoot_K  = 200.0f;
    ...
    const int vtk_every  = static_cast<int>(100.0e-6f / config.dt);
    ...
    // Initialize
    // ==================================================================
    mkdir("output_round8_s2", 0755);

    MultiphysicsSolver solver(config);
    solver.initialize(300.0f, 0.80f);

    // R8 Stage 2: Regularized BGK + EDM, τ override to physical viscosity.
    //   316L ν_phys = 7.6e-7 m²/s
    //   τ_phys = 3·ν·dt/dx² + 0.5 = 3 × 7.6e-7 × 80e-9 / (2e-6)² + 0.5 = 0.5456
    //   ω_phys = 1/τ = 1.834
    // Replaces the legacy 4.3× artificial-viscosity clamp at τ≈0.55. Round 7
    // diagnosis: artificial viscosity attenuates capillary backflow (viscous-
    // diffusion driven) 3-4× more than Marangoni outflow (inviscid). Regularized
    // BGK rejects ghost moments → stable at τ=0.5456 without LES.
    ...
    for (int step = 0; step <= num_steps; ++step) {
        ...
        if (step % vtk_every == 0) {
            ...
            snprintf(filename, sizeof(filename),
                     "output_round8_s2/line_scan_%06d", step);
    ...
    printf("\nOutput: output_round8_s2/line_scan_*.vtk\n");
    printf("============================================================\n\n");
