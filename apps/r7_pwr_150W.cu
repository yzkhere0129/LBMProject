 ...
 *
 * Usage:
 *   mkdir -p output_r7_pwr_150W && ./sim_line_scan_316L
 *   ParaView: Open output_r7_pwr_150W/line_scan_*.vtk
 */
...
int main() {
    ...
    config.nx = 1100;
    config.ny = 75;
    config.nz = 150;  // R7 long: extra headroom for keyhole
    config.dx = 2.0e-6f;
    config.dt = 8.0e-8f;
    ...
    config.laser_power              = 150.0f;  // R7 power sweep
    ...
    const float t_total  = 300.0e-6f;  // R7 power sweep short
    ...
    for (int step = 0; step <= num_steps; ++step) {
        ...
        if (step % vtk_every == 0) {
            ...
            snprintf(filename, sizeof(filename),
                     "output_r7_pwr_150W/line_scan_%06d", step);
    ...
    printf("\nOutput: output_r7_pwr_150W/line_scan_*.vtk\n");
    printf("============================================================\n\n");
