 ...
 *
 * Usage:
 *   mkdir -p output_dry_run_qdt && ./sim_line_scan_316L
 *   ParaView: Open output_dry_run_qdt/line_scan_*.vtk
 */
...
int main() {
    ...
    config.nz = 100;
    config.dx = 2.0e-6f;
    // Track-A dry run: dt/4 = 20 ns. Pushes Ma 0.43 → 0.11 and τ 0.65 → 0.54.
    config.dt = 2.0e-8f;
    ...
    const float t_total  = 20.0e-6f;  // dry run: 1000 steps at dt=20ns
    ...
    const int diag_every = 100;  // dry run: more frequent diag in 1000-step window
    ...
    for (int step = 0; step <= num_steps; ++step) {
        ...
        if (step % vtk_every == 0) {
            ...
            snprintf(filename, sizeof(filename),
                     "output_dry_run_qdt/line_scan_%06d", step);
    ...
    printf("\nOutput: output_dry_run_qdt/line_scan_*.vtk\n");
    printf("============================================================\n\n");
