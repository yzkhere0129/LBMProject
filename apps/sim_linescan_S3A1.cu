...
int main() {
    ...
    // S3-A1: Reduce Marangoni by 50% via csf_multiplier=0.5.
    // Hypothesis: dσ/dT=-4.3e-4 N/(m·K) (pure 316L, Mills 2002) may be
    // 2× too large vs the actual F3D value. At high T near keyhole, sulfur
    // and alloy effects can reduce or even flip dσ/dT. If weaker Marangoni
    // reduces outward radial flow → less mass pushed to side ridges → groove
    // fills (or at least becomes shallower) → evidence this is root cause.
    config.marangoni_csf_multiplier = 0.5f;   // S3-A1: 0.5× (was 1.0)
    ...
    for (int step = 0; step <= num_steps; ++step) {
        ...
        if (step % vtk_every == 0) {
            ...
            snprintf(filename, sizeof(filename),
                     "output_S3A1/line_scan_%06d", step);
